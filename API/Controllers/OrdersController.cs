...
public class OrdersController : BaseApiController
{
        ...
        CreateOrder(OrderCreationDto orderCreationDto)
    {
        ...
        if (order == null) return BadRequest(
          new ApiErrorResponse(400, "We've had a problem creating an order!")
        );
    ...
    [HttpGet]
    public async Task<ActionResult<IReadOnlyList<OrderDto>>>
        GetUserOrders()
    {
        var email = User.RetriveEmail();
    ...
    public async Task<ActionResult<OrderDto>> GetUserOrder(int id)
    {
        ...
        if (order == null) return NotFound(
          new ApiErrorResponse(404, "Couldn't find the order!")
        );
